...
const protocolFactory = new Map<PlatformName, Map<ProtocolName, ProtocolInitializer<PlatformName>>>();
...
export function getProtocolInitializer<P extends PlatformName, PN extends ProtocolName>(
    ...
    if (!protocols)
        throw new Error(
            `No protocols registered for platform ${platform}`,
        );

    const pctr = protocols.get(protocol);
    if (!pctr)
        throw new Error(
            `No protocol registered for ${platform}:${protocol}`,
        );
