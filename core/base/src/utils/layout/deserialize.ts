...
export function deserializeLayout<L extends Layout>(
  ...
  const [decoded, finalOffset] = internalDeserializeLayout(
    layout,
    encoded,
    offset
  );
  ...
  if (consumeAll && finalOffset !== encoded.length)
    throw new Error(
      `encoded data is longer than expected: ${encoded.length} > ${finalOffset}`
    );

  return consumeAll
    ? (decoded as LayoutToType<L>)
    : [decoded as LayoutToType<L>, finalOffset];
}
...
function updateOffset(
  ...
  if (newOffset > encoded.length)
    throw new Error(
      `encoded data is shorter than expected: ${encoded.length} < ${newOffset}`
    );
...
function deserializeLayoutItem(
  ...
  try {
    switch (item.binary) {
      ...
      case "array": {
        ...
        if (item.lengthSize !== undefined) {
          const [length, newOffset] = deserializeUint(
            encoded,
            offset,
            item.lengthSize
          );
          ...
          for (let i = 0; i < length; ++i)
            [ret[i], offset] = internalDeserializeLayout(
              item.layout,
              encoded,
              offset
            );
          ...
          while (offset < encoded.length)
            [ret[ret.length], offset] = internalDeserializeLayout(
              item.layout,
              encoded,
              offset
            );
      ...
      case "bytes": {
        ...
        if (item.custom !== undefined) {
          if (item.custom instanceof Uint8Array) fixedFrom = item.custom;
        ...
        return [
          item.custom !== undefined
            ? (item.custom as narrowedCustom).to(value)
            : value,
          newOffset,
        ];
      }
      case "uint": {
        ...
        if (item.custom !== undefined) {
          if (
            typeof item.custom === "number" ||
            typeof item.custom === "bigint"
          ) {
            checkUintEquals(item.custom, value);
            return [item.custom, newOffset];
          } else if (
            typeof item.custom.from === "number" ||
            typeof item.custom.from === "bigint"
          ) {
    ...
    (e as Error).message = `when deserializing item '${item.name}': ${
      (e as Error).message
    }`;
    throw e;
  }
