...
type FilterFixedItem<I extends LayoutItem> = I extends {
  ...
  ? FixedItemsOfLayout<I["layout"]> extends readonly LayoutItem[]
    ? [FixedItemsOfLayout<I["layout"]>[number]] extends [never]
      ? never
      : {
          readonly [K in keyof I]: K extends "layout"
            ? FixedItemsOfLayout<I["layout"]>
            : I[K];
        }
    : never
  : never;

export type FixedItemsOfLayout<L extends Layout> = L extends readonly [
  infer H extends LayoutItem,
  ...infer T
]
...
export const fixedItemsOfLayout = <L extends Layout>(
  layout: L
): FixedItemsOfLayout<L> =>
...
type FilterDynamicItem<I extends LayoutItem> = I extends {
  ...
  ? DynamicItemsOfLayout<I["layout"]> extends readonly LayoutItem[]
    ? [DynamicItemsOfLayout<I["layout"]>[number]] extends [never]
      ...
      : {
          readonly [K in keyof I]: K extends "layout"
            ? DynamicItemsOfLayout<I["layout"]>
            : I[K];
        }
    : never
  : I;

export type DynamicItemsOfLayout<L extends Layout> = L extends readonly [
  infer H extends LayoutItem,
  ...infer T
]
...
export const dynamicItemsOfLayout = <L extends Layout>(
  layout: L
): DynamicItemsOfLayout<L> =>
  layout.reduce((acc: any, item: any) => {
    if (
      item["custom"] === undefined ||
      !(isPrimitiveType(item["custom"]) || isPrimitiveType(item["custom"].from))
...
export const addFixedValues = <L extends Layout>(
  ...
  for (const item of layout) {
    if (item.binary === "object") {
      ...
      ret[item.name] = subDynamicValues.map((element) =>
        addFixedValues(item.layout, element)
      );
    } else if (
      item.custom !== undefined &&
      (isPrimitiveType(item.custom) ||
        isPrimitiveType((item.custom as { from: any }).from))
    ) {
      if (!(item as { omit?: boolean })?.omit)
        ret[item.name] = isPrimitiveType(item.custom)
          ? item.custom
          : item.custom.to;
    } else
