...
export function serializeLayout<L extends Layout>(
  ...
  for (let i = 0; i < layout.length; ++i)
    offset = serializeLayoutItem(
      layout[i],
      data[layout[i].name as keyof typeof data],
      ret,
      offset
    );
...
const calcLayoutSize = (
  ...
  layout.reduce((acc: number, item: LayoutItem) => {
    switch (item.binary) {
      ...
      case "array": {
        if (item.lengthSize !== undefined) acc += item.lengthSize;
      ...
      case "bytes": {
        ...
        if ("size" in item && item.size !== undefined) return acc + item.size;
...
export function serializeUint(
  ...
  if (
    bytes > numberMaxSize &&
    typeof val === "number" &&
    val >= 2 ** (numberMaxSize * 8)
  )
    throw new Error(
      `Value ${val} is too large to be safely converted into an integer`
    );
  ...
  for (let i = 0; i < bytes; ++i)
    encoded[offset + i] = Number(
      (BigInt(val) >> BigInt(8 * (bytes - i - 1))) & 0xffn
    );
...
function serializeLayoutItem(
  ...
  try {
    switch (item.binary) {
      ...
      case "uint": {
        const value = (() => {
          ...
          type narrowedCustom =
            | CustomConversion<number, any>
            | CustomConversion<bigint, any>;
          return item.custom !== undefined
            ? (item.custom as narrowedCustom).from(data)
            : data;
        })();
    ...
    (e as Error).message = `when serializing item '${item.name}': ${
      (e as Error).message
    }`;
    throw e;
  }
