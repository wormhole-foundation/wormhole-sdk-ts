...
export type UintSizeToPrimitive<Size extends number> = Size extends NumberSize
  ? number
  : bigint;
...
type NameOrOmitted<T extends { name: PropertyKey }> = T extends { omit: true }
  ? never
  : T["name"];
...
export type LayoutItemToType<I extends LayoutItem> = [I] extends [
  ArrayLayoutItem
]
