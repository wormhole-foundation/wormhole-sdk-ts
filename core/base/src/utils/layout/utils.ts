...
export const isNumType = (x: any): x is NumType => typeof x === "number" || typeof x === "bigint";

export const isBytesType = (x: any): x is BytesType => x instanceof Uint8Array;

export const isPrimitiveType = (x: any): x is PrimitiveType => isNumType(x) || isBytesType(x);
...
export const isFixedUintConversion = (
  custom: any,
): custom is FixedConversion<number, any> | FixedConversion<bigint, any> =>
...
export const isFixedPrimitiveConversion = (
  custom: any,
): custom is
  | FixedConversion<number, any>
  | FixedConversion<bigint, any>
  | FixedConversion<BytesType, any> =>
...
export type BytesBase = { readonly name: string } & Omit<
  BytesLayoutItem,
  "binary" | "custom" | "layout"
>;

export type CustomizableBytesReturn<
  B extends BytesBase,
  P extends CustomizableBytes,
> = CombineObjects<
  B,
  P extends undefined
...
export const customizableBytes = <const B extends BytesBase, const C extends CustomizableBytes>(
  base: B,
  spec?: C,
) =>
  ({
    ...
    ...(() => {
      ...
      if (isLayout(spec)) return { layout: spec };
...
export function findIdLayoutPair(item: SwitchLayoutItem, data: any) {
  ...
  return (item.layouts as readonly any[]).find(
    ([idOrConversionId]) =>
