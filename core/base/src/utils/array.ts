...
export type Flatten<T extends RoArray> = T extends readonly [
  infer Head,
  ...infer Tail
]
...
export type IsFlat<T extends RoArray> = T extends readonly [
  infer Head,
  ...infer Tail
]
  ? Head extends RoArray
...
export type AllSameLength<
  T extends RoArray2D,
  L extends number
> = T extends readonly [
  infer Head extends RoArray,
  ...infer Tail extends RoArray2D
]
...
export type IsRectangular<T extends RoArray> = T extends RoArray2D
  ? T extends readonly [
      infer Head extends RoArray,
      ...infer Tail extends RoArray2D
    ]
...
export const column = <A extends RoArray2D, I extends number>(
  tupArr: A,
  index: I
...
export const zip = <Args extends RoArray2D>(arr: Args) =>
  range(arr[0].length).map((col) =>
    range(arr.length).map((row) => arr[row][col])
  ) as unknown as [Zip<Args>] extends [never] ? RoArray2D : Zip<Args>;

export type OnlyIndexes<E extends RoArray, I extends IndexEs> = I extends number
  ? OnlyIndexes<E, [I]>
  : I extends readonly [
      infer Head extends number,
      ...infer Tail extends RoArray<number>
    ]
...
export type ExcludeIndexes<
  T extends RoArray,
  C extends IndexEs
> = ExcludeIndexesImpl<
  Entries<T>,
  C extends RoArray<number> ? ParseNumber<keyof IndexEs> : C
>;

export type Cartesian<L, R> = L extends RoArray
  ? Flatten<
      [
        ...{
