...
describe("Finality tests", function () {
  ...
  test("Estimates rounds from instant consistency level", () => {
    expect(consistencyLevelToBlock("Algorand", ConsistencyLevels.Immediate, fromBlock)).toEqual(
      fromBlock,
    );
  ...
  test("Estimates rounds from finalized consistency level", () => {
    ...
    // 100 + 0 (instant)
    expect(consistencyLevelToBlock("Algorand", ConsistencyLevels.Finalized, fromBlock)).toEqual(
      fromBlock,
    );
