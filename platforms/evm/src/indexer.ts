...
export class GoldRushClient {
  ...
  async getBalances(
    ...
    if (json.error) {
      throw new Error(
        `GoldRush API error: ${
          json.error_message || json.error || 'Unknown error'
        }`,
      );
    }
...
export class AlchemyClient {
  ...
  async getBalances(
    ...
    if (data.result && data.result.tokenBalances) {
      for (let item of data.result.tokenBalances) {
        // Only include tokens with non-zero balance
        // The balance is returned as hex string (0x prefixed)
        const balance = BigInt(item.tokenBalance || item.balance || '0x0');
        if (balance > 0n) {
