...
import {
  ...
  textToUint8Array,
  uint8ArrayToHex,
} from "./utilities"; // TODO: Replace these to remove dependency on Buffer
...
export async function submitVAAHeader(
  ...
  for (let nt = 0; nt < numTxns; nt++) {
    ...
    for (let i = 0; i < numSigsThisTxn; i++) {
      ...
      const idx = sigs[i * SIG_LEN]!; // Added non-null assertion
...
export async function _submitVAAAlgorand(
  ...
  if (meta === "TokenBridge RegisterChain" || meta === "TokenBridge UpgradeContract") {
    txs.push({
      tx: makeApplicationCallTxnFromObject({
        appArgs: [textToUint8Array("governance"), vaa],
        accounts: accts,
  ...
  if (meta === "TokenBridge Attest") {
    ...
    txs[txs.length - 1]!.tx.fee = txs[txs.length - 1]!.tx.fee * 2; // Added non-null assertion.  QUESTIONBW: There are like 3 different ways of adjusting fees in various functions--this should be standardized
  }

  if (
    (meta === "TokenBridge Transfer" || meta === "TokenBridge Transfer With Payload") &&
    parsedVAA.Contract !== undefined
  ) {
    let foreignAssets: number[] = [];
    let a: number = 0;
    ...
    if (
      ...
      txs[txs.length - 1]!.tx.fee = txs[txs.length - 1]!.tx.fee * 2; // Added non-null assertions
    else txs[txs.length - 1]!.tx.fee = txs[txs.length - 1]!.tx.fee * 3;

    if (meta === "TokenBridge Transfer With Payload") {
      txs[txs.length - 1]!.tx.appForeignApps = [aid]; // Added non-null assertion

      let m = ABIMethod.fromSignature("portal_transfer(byte[])byte[]");

      txs.push({
        tx: makeApplicationCallTxnFromObject({
          appArgs: [m.getSelector(), (m.args[0]!.type as ABIType).encode(vaa)], // Added non-null assertion
          appIndex: aid,
          foreignAssets: foreignAssets,
          from: senderAddr,
          onComplete: OnApplicationComplete.NoOpOC,
...
export function _parseVAAAlgorand(vaa: Uint8Array): ParsedVAA {
  ...
  if (
    !Buffer.compare(
      extract3(buf, off, 32),
      Buffer.from("000000000000000000000000000000000000000000546f6b656e427269646765", "hex"),
    )
  ) {
    ...
    !Buffer.compare(
      extract3(buf, off, 32),
      Buffer.from("00000000000000000000000000000000000000000000000000000000436f7265", "hex"),
    )
  ) {
