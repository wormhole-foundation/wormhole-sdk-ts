...
export class CosmwasmPlatform<N extends Network> extends PlatformContext<N, CosmwasmPlatformType> {
  ...
  async getLatestBlock(rpc: CosmWasmClient): Promise<number> {
    return rpc.getHeight();
  }
  async getLatestFinalizedBlock(rpc: CosmWasmClient): Promise<number> {
    throw new Error("not implemented");
  }

  static chainFromChainId(chainMoniker: string): [Network, PlatformToChains<CosmwasmPlatformType>] {
