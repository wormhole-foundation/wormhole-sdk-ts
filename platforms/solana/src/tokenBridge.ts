...
import { TokenBridgeContract } from '@certusone/wormhole-sdk/lib/cjs/solana/types/tokenBridge';
...
const addFrom = (txReq: any, from: string) => ({
  ...txReq,
  from,
});
const addChainId = (txReq: any, chainId: bigint) => ({
  ...txReq,
  chainId,
});
const unusedNonce = 0;
...
export class SolanaTokenBridge implements TokenBridge<'Solana'> {
  ...
  async isWrappedAsset(token: UniversalOrNative<'Solana'>): Promise<boolean> {
    return this.tokenBridge.isWrappedAsset(token.toString());
  }
