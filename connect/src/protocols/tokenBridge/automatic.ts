...
type PN = "AutomaticTokenBridge";
export class AutomaticTokenTransfer<N extends Network>
  ...
  implements WormholeTransfer<PN>
{
  ...
  async initiateTransfer(signer: Signer): Promise<TxHash[]> {
    ...
    const originTxs = await AutomaticTokenTransfer.transfer(this.fromChain, this.transfer, signer);
  ...
  async completeTransfer(signer: Signer): Promise<TxHash[]> {
    ...
    const destinationTxs = await AutomaticTokenTransfer.redeem<N>(
      this.toChain,
      vaa as AutomaticTokenBridge.VAA,
      signer,
    );
  ...
  static async transfer<N extends Network>(
    ...
    const tb = await chainCtx.getAutomaticTokenBridge();
